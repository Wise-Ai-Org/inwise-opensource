...
/**
 * Stitch multiple WAV buffers of the same format into one continuous WAV.
 * Used when one meeting's audio arrives as several segments (duplicate badge
 * windows, VAD splits) so it is processed as a single recording.
 */
export function stitchWavBuffers(wavs: Buffer[]): Buffer {
  ...
  for (const wav of wavs) {
    ...
    pcmChunks.push(wav.subarray(dataOffset, dataOffset + dataSize));
  }
